...
describe("simpleWithDefaultCollection",function(){
    ...
    describe("simple Queries",function(){
        ...
        it('list all documents matching an example, we tests that passing skip and limit in the function beats the global setting',function(done){
            var opt = {"skip" : 0, "limit" : 4}
            db.simple.skip(1).limit(2).example({"income" : 2100}, opt, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
        ...
        it('list all documents matching an example, without options',function(done){
            db.simple.skip(1).limit(2).example({"income" : 2100},function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
                    ret.result.length.should.equal(2);
                    message.status.should.equal(201);
        ...
        it('remove all documents matching an example.',function(done){
            ...
            db.simple.removeByExample({"income" : 2900}, opt, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
        ...
        it('remove all documents matching an example.',function(done){
            db.simple.removeByExample({"income" : 2900}, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
        ...
        it('replace all documents matching an example.',function(done){
            var opt = {"waitForSync" : true, "limit" : 1}
            db.simple.replaceByExample( {"age" : 30}, {"age" : 31, "married" : true}, opt, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
        ...
        it('replace all documents matching an example, without options',function(done){
            db.simple.replaceByExample({"age" : 30}, {"age" : 31, "married" : true}, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
        ...
        it('update all documents matching an example.',function(done){
            ...
            db.simple.updateByExample({"age" : 31}, {"married" : false}, opt, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
        ...
        it('return the first documents matching a given example.',function(done){
            var opt = {"skip" : 0, "limit" : 4}
            db.simple.firstByExample( {"income" : 2100}, opt, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
                    ret.should.have.property("document");
                    message.status.should.equal(200);
        ...
        it('return the first documents matching a given example, without options',function(done){
            db.simple.firstByExample({"income" : 2100}, function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
                    ret.should.have.property("document");
                    message.status.should.equal(200);
        ...
        it('use the skip list index for a range query',function(done){
            db.simple.range("age", 23, 29, {closed : true},  function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
                    ret.result.length.should.equal(2);
                    message.status.should.equal(201);
        ...
        it('use the fulltext index for a fulltext query with options',function(done){
            db.simple.fulltext("birthplace", "munich", {"limit" : 2},    function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
                    ret.result.length.should.equal(2);
                    message.status.should.equal(201);
        ...
        it('use the geo index for a near query on location',function(done){
            ...
            db.simple.skip(undefined).limit(undefined).near(15, 15 , {geo : index, distance : "distance"},  function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
                    ret.result.length.should.equal(4);
                    message.status.should.equal(201);
        ...
        it('use the geo index for a within query on location',function(done){
            var index = getIndexByType("GeoCollection", "geo1");
            db.simple.within(15, 15 , 787593,  {geo : index, distance : "distance"},  function(err,ret, message){
                check( done, function () {
                    ret.error.should.equal(false);
                    ret.result.length.should.equal(3);
                    message.status.should.equal(201);
