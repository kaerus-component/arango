...
function Connection(){
    ...
    for(var i = 0, connection; connection = arguments[i]; i++) {
        if(typeof connection === 'object') {
            ...
            var conn = url.parse(connection);           
            if(conn.host) utils.extend(arango.server,conn.host);
            arango.server.port = parseInt(arango.server.port,10);
    ...
    /* Basic authorization */
    if(arango.server.username) {
        arango.server.headers = arango.server.headers||{};
        arango.server.headers.authorization = 'Basic ' + 
            base64.encode(arango.server.username + ':' + arango.server.password);  
    }
