...
/* note: submit is global for all instances */
var submit = {};
...
var ActionAPI = {
    ...
    "define": function(o,f,reload) {
        ...
        if(inject){
            ...
            /* inject the function as arangothis action callback */
            this.db.use(":_routing").document.create(route).then(function(res){
        ...
        function action() {
            ...
            return db[method].apply(db,args).then(
                o.result,o.error
            );
        }
    ...
    "undefine": function(name) {
        ...
        if(submit[name].route) {
            this.db.document.delete(submit[name].route).done();
        }    
