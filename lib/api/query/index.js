...
Aql.bindKeyword = function(key,proto){
...
Aql.bindFunction = function(name,proto){
...
(function(){
	...
	Aql.bindKeyword(keywords[keyword], Aql.prototype);
    }

    // bind graph functions
    for(var graphFunction in graphFunctions){
	Aql.bindFunction(graphFunctions[graphFunction],Aql.prototype.in);
	Aql.bindFunction(graphFunctions[graphFunction],Aql.prototype.return);
    }
	...
	return struct._string || keywords.concat(graphFunctions)
	    .filter(function (key) {
		return !!struct[key];
	    }).map(function (q) {
